    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (input[0] != 'q')
            {
                ...
                float distanceBetweenTwoPoint = (float)Math.Sqrt(deltaX * deltaX + deltaY * deltaY);
                float angle = ((float)Math.Atan((float)deltaY / (float)deltaX)) *180 / (float)Math.PI;
                Console.WriteLine(distanceBetweenTwoPoint + " " + angle);
